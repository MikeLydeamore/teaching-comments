...
export const neonStore: QwtStore = {
  ...
  async getActivePoll(code) { const sessionCode=normalizeSessionCode(code)||"demo-lecture";const rows=await query(`SELECT ${POLL_COLUMNS} FROM qwt_polls WHERE session_code=$1 AND status='active' ORDER BY started_at DESC LIMIT 1`,[sessionCode]);return rows[0]?pollFromRow(rows[0]):null; },
  ...
  async startPoll(code,question,selectionMode,labels,correctOptionIndexes,durationSeconds) { const sessionRow=await getSessionRow(code);if(!sessionRow)return null;const session=sessionFromRow(sessionRow);if(!session.isOpen)throw new Error("This session is closed.");const d=validatePollDefinition(question,selectionMode,labels,durationSeconds);const timestamp=now();const options=d.optionLabels.map((label,position)=>({id:randomUUID(),label,position}));const correctOptionIds=validateCorrectOptionIndexes(d.selectionMode,d.optionLabels,correctOptionIndexes).map((index)=>options[index].id);const rows=await query(`WITH ended AS (UPDATE qwt_polls SET status='ended',ended_at=$2,updated_at=$2 WHERE session_code=$1 AND status='active') INSERT INTO qwt_polls (session_code,question,selection_mode,options,correct_option_ids,solution_revealed,status,duration_seconds,started_at,ends_at,created_at,updated_at) VALUES ($1,$3,$4,$5::jsonb,$6::jsonb,false,'active',$7,$2,$8,$2,$2) RETURNING ${POLL_COLUMNS}`,[session.id,timestamp,d.question,d.selectionMode,JSON.stringify(options),JSON.stringify(correctOptionIds),d.durationSeconds,new Date(Date.parse(timestamp)+d.durationSeconds*1000).toISOString()]);return pollFromRow(rows[0]); },
  async extendPoll(id,seconds) { const extension=validatePollExtension(seconds);const row=await getPollRow(id);if(!row)return null;const poll=pollFromRow(row);if(poll.status!=="active")throw new Error("This poll has been ended.");if(Date.parse(poll.endsAt)<=Date.now())throw new Error("This poll timer has ended.");const base=Math.max(Date.now(),Date.parse(poll.endsAt));const rows=await query(`UPDATE qwt_polls SET duration_seconds=duration_seconds+$2,ends_at=$3,updated_at=$4 WHERE id=$1::uuid AND status='active' RETURNING ${POLL_COLUMNS}`,[id,extension,new Date(base+extension*1000).toISOString(),now()]);return rows[0]?pollFromRow(rows[0]):null; },
  ...
  async revealPollSolution(id) { const rows=await query(`UPDATE qwt_polls SET solution_revealed=true,updated_at=$2 WHERE id=$1::uuid RETURNING ${POLL_COLUMNS}`,[id,now()]);return rows[0]?pollFromRow(rows[0]):null; },
