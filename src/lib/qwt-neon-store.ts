...
function pollFromRow(row: Row): SessionPoll {
  ...
  return {
    ...
    options: (Array.isArray(options) ? options : []) as PollOption[], status: text(row, "status") as SessionPoll["status"], durationSeconds: number(row, "duration_seconds"),
...
const POLL_COLUMNS = "id, session_code, question, selection_mode, options, status, duration_seconds, started_at, ends_at, ended_at, created_at, updated_at";

async function getSessionRow(code: string) {
  const normalized = normalizeSessionCode(code);
  if (!normalized) return null;
  return (await query(`SELECT ${SESSION_COLUMNS} FROM qwt_sessions WHERE id = $1 LIMIT 1`, [normalized]))[0] ?? null;
}
...
async function getSessionInSpaceRow(spaceCode: string, code: string) {
  ...
  return (await query(`SELECT ${SESSION_COLUMNS} FROM qwt_sessions WHERE space_code = $1 AND code = $2 LIMIT 1`, [space, session]))[0] ?? null;
}

async function getPollRow(id: string) {
  return (await query(`SELECT ${POLL_COLUMNS} FROM qwt_polls WHERE id = $1 LIMIT 1`, [id]))[0] ?? null;
}

async function ensurePromptHistory(session: Session) {
  await query(
    `INSERT INTO qwt_prompt_history (session_code, prompt, started_at)
     SELECT $1, $2, $3 WHERE NOT EXISTS (SELECT 1 FROM qwt_prompt_history WHERE session_code = $1)`,
...
async function groupQuestionRow(id: string, voterId = "") {
  const rows = await query(
    `SELECT q.*, COUNT(v.question_id)::integer AS vote_count,
     ...
     FROM qwt_group_questions q LEFT JOIN qwt_group_question_votes v ON v.question_id = q.id
     WHERE q.id = $1 GROUP BY q.id`, [id, voterId],
  );
  return rows[0] ?? null;
}

export const neonStore: QwtStore = {
  async createTeacherSpace(code, name, pinHash) {
    ...
    try {
      const rows = await query("INSERT INTO qwt_teacher_spaces (code, name, pin_hash) VALUES ($1, $2, $3) RETURNING code, name, pin_hash, created_at", [normalized, validateTeacherSpaceName(name), validateTeacherSpacePinHash(pinHash)]);
  ...
  async getTeacherSpace(code) {
    ...
    const rows = await query("SELECT code, name, pin_hash, created_at FROM qwt_teacher_spaces WHERE code = $1 LIMIT 1", [normalized]);
    return rows[0] ? teacherSpaceFromRow(rows[0]) : null;
  },
  async listTeacherSpaces() {
    const rows = await query("SELECT code, name, created_at FROM qwt_teacher_spaces ORDER BY name ASC");
  ...
  async updateTeacherSpacePinHash(code, pinHash) {
    const normalized = normalizeSpaceCode(code); if (!normalized) return null;
    const rows = await query("UPDATE qwt_teacher_spaces SET pin_hash = $2 WHERE code = $1 RETURNING code, name, pin_hash, created_at", [normalized, validateTeacherSpacePinHash(pinHash)]);
    return rows[0] ? teacherSpaceFromRow(rows[0]) : null;
  },
  ...
  async getOrCreateSessionInSpace(spaceCode, code) {
    ...
    const space = await query("SELECT 1 FROM qwt_teacher_spaces WHERE code = $1", [spaceCodeNormalized]);
    if (!space.length) return null;
    const timestamp = now(); const id = randomUUID();
    const rows = await query(
      `INSERT INTO qwt_sessions (id, code, space_code, title, prompt, is_open, group_questions_screening_enabled, submissions_screening_enabled, text_input_enabled, gif_input_enabled, drawing_input_enabled, image_input_enabled, created_at, prompt_updated_at, timer_duration_seconds, timer_ends_at)
  ...
  async listSessions(spaceCode) {
    ...
    const rows = normalized ? await query(`SELECT ${SESSION_COLUMNS} FROM qwt_sessions WHERE space_code = $1 ORDER BY created_at DESC`, [normalized]) : await query(`SELECT ${SESSION_COLUMNS} FROM qwt_sessions ORDER BY created_at DESC`);
  ...
  async updateSession(code, patch) {
    ...
    if (changed) {
      await query(
        `WITH closed AS (UPDATE qwt_prompt_history SET ended_at=$2 WHERE session_code=$1 AND ended_at IS NULL)
         INSERT INTO qwt_prompt_history (session_code,prompt,started_at) VALUES ($1,$3,$2)`,
  ...
  async listPromptHistory(code) {
    ...
    return (await query("SELECT id, session_code, prompt, started_at, ended_at FROM qwt_prompt_history WHERE session_code = $1 ORDER BY started_at DESC", [session.id])).map(promptHistoryFromRow);
  },
  async listSubmissions(code, options = {}) {
    ...
    if (options.promptHistoryId) {
      const rows = await query("SELECT id, session_code, prompt, started_at, ended_at FROM qwt_prompt_history WHERE session_code = $1 AND id = $2", [sessionCode, options.promptHistoryId]);
    ...
    return (await query(`SELECT ${SUBMISSION_COLUMNS} FROM qwt_submissions WHERE ${clauses.join(" AND ")} ORDER BY created_at DESC`, values)).map(submissionFromRow);
  },
  async addSubmission(code, input) {
    ...
    const timestamp = now(); const rows = await query(
      `INSERT INTO qwt_submissions (id,session_code,student_name,text,drawing_data,gif_data,image_data,status,starred,flagged,version,archived_at,created_at,updated_at)
  ...
  async getSubmission(id) { const rows = await query(`SELECT ${SUBMISSION_COLUMNS} FROM qwt_submissions WHERE id = $1::uuid`, [id]); return rows[0] ? submissionFromRow(rows[0]) : null; },
  async updateSubmission(sessionCode, id, patch) {
    const currentRows = await query(`SELECT ${SUBMISSION_COLUMNS} FROM qwt_submissions WHERE id=$1::uuid AND session_code=$2`, [id,sessionCode]); if (!currentRows[0]) return null;
    ...
    const rows = await query(`UPDATE qwt_submissions SET text=CASE WHEN $3 THEN $4 ELSE text END,status=COALESCE($5,status),starred=COALESCE($6,starred),flagged=COALESCE($7,flagged),version=version+1,updated_at=$8 WHERE id=$1::uuid AND session_code=$2 RETURNING ${SUBMISSION_COLUMNS}`,[id,sessionCode,hasText,nextText,normalized.status ?? null,typeof normalized.starred === "boolean" ? normalized.starred : null,typeof normalized.flagged === "boolean" ? normalized.flagged : null,now()]);
    return rows[0] ? submissionFromRow(rows[0]) : null;
  },
  async getSessionStats(code) { const sessionCode=normalizeSessionCode(code)||"demo-lecture"; return calculateStats((await query(`SELECT ${SUBMISSION_COLUMNS} FROM qwt_submissions WHERE session_code=$1 AND archived_at IS NULL`,[sessionCode])).map(submissionFromRow)); },
  async listQuestionBank(code) { const sessionCode=normalizeSessionCode(code)||"demo-lecture"; return (await query("SELECT id,session_code,title,text,created_at,updated_at FROM qwt_question_bank WHERE session_code=$1 ORDER BY title ASC",[sessionCode])).map((r)=>({id:text(r,"id"),sessionCode:text(r,"session_code"),title:text(r,"title")||text(r,"text"),text:text(r,"text"),createdAt:text(r,"created_at"),updatedAt:text(r,"updated_at")})); },
  async addQuestionToBank(code, question, title) { const row=await getSessionRow(code); if(!row)return null; const timestamp=now(); const rows=await query("INSERT INTO qwt_question_bank (session_code,title,text,created_at,updated_at) VALUES ($1,$2,$3,$4,$4) RETURNING id,session_code,title,text,created_at,updated_at",[text(row,"id"),validateQuestionTitle(title,question),validateQuestionText(question),timestamp]); const r=rows[0]; return {id:text(r,"id"),sessionCode:text(r,"session_code"),title:text(r,"title"),text:text(r,"text"),createdAt:text(r,"created_at"),updatedAt:text(r,"updated_at")}; },
  async getQuestionFromBank(id) { const rows=await query("SELECT id,session_code,title,text,created_at,updated_at FROM qwt_question_bank WHERE id=$1::uuid",[id]); const r=rows[0]; return r?{id:text(r,"id"),sessionCode:text(r,"session_code"),title:text(r,"title")||text(r,"text"),text:text(r,"text"),createdAt:text(r,"created_at"),updatedAt:text(r,"updated_at")}:null; },
  async deleteQuestionFromBank(sessionCode,id) { return (await query("DELETE FROM qwt_question_bank WHERE id=$1::uuid AND session_code=$2 RETURNING id",[id,sessionCode])).length>0; },
  async listPollQuestionBank(code) { const sessionCode=normalizeSessionCode(code)||"demo-lecture"; return (await query("SELECT id,session_code,title,question,selection_mode,options,created_at,updated_at FROM qwt_poll_question_bank WHERE session_code=$1 ORDER BY title ASC",[sessionCode])).map((r)=>({id:text(r,"id"),sessionCode:text(r,"session_code"),title:text(r,"title")||text(r,"question"),question:text(r,"question"),selectionMode:text(r,"selection_mode") as "single"|"multiple",options:(json(r,"options") as string[])??[],createdAt:text(r,"created_at"),updatedAt:text(r,"updated_at")})); },
  async addPollQuestionToBank(code,title,question,selectionMode,options) { const row=await getSessionRow(code); if(!row)return null; const d=validatePollQuestionDefinition(question,selectionMode,options); const timestamp=now(); const rows=await query("INSERT INTO qwt_poll_question_bank (session_code,title,question,selection_mode,options,created_at,updated_at) VALUES ($1,$2,$3,$4,$5::jsonb,$6,$6) RETURNING id,session_code,title,question,selection_mode,options,created_at,updated_at",[text(row,"id"),validatePollQuestionTitle(title,d.question),d.question,d.selectionMode,JSON.stringify(d.optionLabels),timestamp]); const r=rows[0]; return {id:text(r,"id"),sessionCode:text(r,"session_code"),title:text(r,"title"),question:text(r,"question"),selectionMode:text(r,"selection_mode") as "single"|"multiple",options:json(r,"options") as string[],createdAt:text(r,"created_at"),updatedAt:text(r,"updated_at")}; },
  async deletePollQuestionFromBank(code,id) { return (await query("DELETE FROM qwt_poll_question_bank WHERE id=$1::uuid AND session_code=$2 RETURNING id",[id,normalizeSessionCode(code)||"demo-lecture"])).length>0; },
  async listGroupQuestions(code,voterId,options={}) { const sessionCode=normalizeSessionCode(code)||"demo-lecture"; const voter=voterId?validateGroupQuestionVoterId(voterId):""; const values:unknown[]=[sessionCode,voter]; const clauses=["q.session_code=$1"]; if(!options.includeAnswered)clauses.push("q.is_answered=false");if(!options.includeHidden)clauses.push("q.is_visible=true");if(!options.includeArchived)clauses.push("q.archived_at IS NULL"); const rows=await query(`SELECT q.*,COUNT(v.question_id)::integer AS vote_count,CASE WHEN $2::text='' THEN false ELSE BOOL_OR(v.voter_id=$2) END AS has_voted FROM qwt_group_questions q LEFT JOIN qwt_group_question_votes v ON v.question_id=q.id WHERE ${clauses.join(" AND ")} GROUP BY q.id ORDER BY q.is_answered ASC, COUNT(v.question_id) DESC, q.created_at DESC`,values);return rows.map((r)=>groupQuestionFromRow(r,voter)); },
  async addGroupQuestion(code,question,studentName) { const s=await getSessionRow(code);if(!s)return null;const session=sessionFromRow(s);if(!session.isOpen)throw new Error("This session is closed.");const timestamp=now();const rows=await query(`INSERT INTO qwt_group_questions (session_code,student_name,text,is_answered,is_visible,archived_at,created_at,updated_at) VALUES ($1,$2,$3,false,$4,NULL,$5,$5) RETURNING ${GROUP_QUESTION_COLUMNS}`,[session.id,normalizeStudentName(studentName??""),validateGroupQuestionText(question),!session.groupQuestionsScreeningEnabled,timestamp]);return groupQuestionFromRow({...rows[0],vote_count:0}); },
  async getGroupQuestion(id) { const r=await groupQuestionRow(id);return r?groupQuestionFromRow(r):null; },
  async upvoteGroupQuestion(id,voterId) { const voter=validateGroupQuestionVoterId(voterId);const current=await groupQuestionRow(id,voter);if(!current)return null;const session=await getSessionRow(text(current,"session_code"));if(!sessionFromRow(session??{}).isOpen)throw new Error("This session is closed.");await query("INSERT INTO qwt_group_question_votes (question_id,voter_id) VALUES ($1::uuid,$2) ON CONFLICT (question_id,voter_id) DO NOTHING",[id,voter]);const result=await groupQuestionRow(id,voter);return result?groupQuestionFromRow(result,voter):null; },
  async unvoteGroupQuestion(id,voterId) { const voter=validateGroupQuestionVoterId(voterId);const current=await groupQuestionRow(id,voter);if(!current)return null;const session=await getSessionRow(text(current,"session_code"));if(!sessionFromRow(session??{}).isOpen)throw new Error("This session is closed.");await query("DELETE FROM qwt_group_question_votes WHERE question_id=$1::uuid AND voter_id=$2",[id,voter]);const result=await groupQuestionRow(id,voter);return result?groupQuestionFromRow(result,voter):null; },
  async setGroupQuestionAnswered(sessionCode,id,isAnswered) { const rows=await query(`UPDATE qwt_group_questions SET is_answered=$3,updated_at=$4 WHERE id=$1::uuid AND session_code=$2 RETURNING ${GROUP_QUESTION_COLUMNS}`,[id,sessionCode,isAnswered,now()]);return rows[0]?groupQuestionFromRow({...rows[0],vote_count:0}):null; },
  async setGroupQuestionVisible(sessionCode,id,isVisible) { const rows=await query(`UPDATE qwt_group_questions SET is_visible=$3,updated_at=$4 WHERE id=$1::uuid AND session_code=$2 RETURNING ${GROUP_QUESTION_COLUMNS}`,[id,sessionCode,isVisible,now()]);return rows[0]?groupQuestionFromRow({...rows[0],vote_count:0}):null; },
  async getActivePoll(code) { const sessionCode=normalizeSessionCode(code)||"demo-lecture";const rows=await query(`SELECT ${POLL_COLUMNS} FROM qwt_polls WHERE session_code=$1 AND status='active' AND ends_at>$2 ORDER BY started_at DESC LIMIT 1`,[sessionCode,now()]);return rows[0]?pollFromRow(rows[0]):null; },
  async getLatestPoll(code) { const sessionCode=normalizeSessionCode(code)||"demo-lecture";const rows=await query(`SELECT ${POLL_COLUMNS} FROM qwt_polls WHERE session_code=$1 ORDER BY started_at DESC LIMIT 1`,[sessionCode]);return rows[0]?pollFromRow(rows[0]):null; },
  async getPollHistory(code) { const sessionCode=normalizeSessionCode(code)||"demo-lecture";const polls=(await query(`SELECT ${POLL_COLUMNS} FROM qwt_polls WHERE session_code=$1 ORDER BY started_at DESC`,[sessionCode])).map(pollFromRow);return Promise.all(polls.map(async poll=>{const responses=await query("SELECT option_ids FROM qwt_poll_responses WHERE poll_id=$1::uuid",[poll.id]);return {poll,responseCount:responses.length,options:poll.options.map(option=>({...option,responseCount:responses.filter(r=>Array.isArray(json(r,"option_ids"))&&(json(r,"option_ids") as unknown[]).includes(option.id)).length}))};})); },
  async getPoll(id) { const r=await getPollRow(id);return r?pollFromRow(r):null; },
  async startPoll(code,question,selectionMode,labels,durationSeconds) { const sessionRow=await getSessionRow(code);if(!sessionRow)return null;const session=sessionFromRow(sessionRow);if(!session.isOpen)throw new Error("This session is closed.");const d=validatePollDefinition(question,selectionMode,labels,durationSeconds);const timestamp=now();const options=d.optionLabels.map((label,position)=>({id:randomUUID(),label,position}));const rows=await query(`WITH ended AS (UPDATE qwt_polls SET status='ended',ended_at=$2,updated_at=$2 WHERE session_code=$1 AND status='active') INSERT INTO qwt_polls (session_code,question,selection_mode,options,status,duration_seconds,started_at,ends_at,created_at,updated_at) VALUES ($1,$3,$4,$5::jsonb,'active',$6,$2,$7,$2,$2) RETURNING ${POLL_COLUMNS}`,[session.id,timestamp,d.question,d.selectionMode,JSON.stringify(options),d.durationSeconds,new Date(Date.parse(timestamp)+d.durationSeconds*1000).toISOString()]);return pollFromRow(rows[0]); },
  async extendPoll(id,seconds) { const extension=validatePollExtension(seconds);const row=await getPollRow(id);if(!row)return null;const poll=pollFromRow(row);if(poll.status!=="active")throw new Error("This poll has been ended.");const base=Math.max(Date.now(),Date.parse(poll.endsAt));const rows=await query(`UPDATE qwt_polls SET duration_seconds=duration_seconds+$2,ends_at=$3,updated_at=$4 WHERE id=$1::uuid AND status='active' RETURNING ${POLL_COLUMNS}`,[id,extension,new Date(base+extension*1000).toISOString(),now()]);return rows[0]?pollFromRow(rows[0]):null; },
  async endPoll(id) { const timestamp=now();const rows=await query(`UPDATE qwt_polls SET status='ended',ended_at=$2,updated_at=$2 WHERE id=$1::uuid RETURNING ${POLL_COLUMNS}`,[id,timestamp]);return rows[0]?pollFromRow(rows[0]):null; },
  async getPollResponse(pollId,participantId) { const participant=validatePollParticipantId(participantId);const rows=await query("SELECT poll_id,participant_id,option_ids,updated_at FROM qwt_poll_responses WHERE poll_id=$1::uuid AND participant_id=$2",[pollId,participant]);const r=rows[0];return r?{pollId:text(r,"poll_id"),participantId:text(r,"participant_id"),optionIds:(json(r,"option_ids") as string[])??[],updatedAt:text(r,"updated_at")}:null; },
  async savePollResponse(pollId,participantId,optionIds) { const participant=validatePollParticipantId(participantId);const pollRow=await getPollRow(pollId);if(!pollRow)return null;const poll=pollFromRow(pollRow);const session=await getSessionRow(poll.sessionCode);if(!sessionFromRow(session??{}).isOpen)throw new Error("This session is closed.");if(poll.status!=="active"||Date.parse(poll.endsAt)<=Date.now())throw new Error("This poll has ended.");const selected=[...new Set(optionIds)];const allowed=new Set(poll.options.map(o=>o.id));if(selected.some(id=>!allowed.has(id)))throw new Error("That poll answer could not be found.");if(poll.selectionMode==="single"&&selected.length>1)throw new Error("Choose one answer for this poll.");const rows=await query("INSERT INTO qwt_poll_responses (poll_id,participant_id,option_ids,updated_at) VALUES ($1::uuid,$2,$3::jsonb,$4) ON CONFLICT (poll_id,participant_id) DO UPDATE SET option_ids=EXCLUDED.option_ids,updated_at=EXCLUDED.updated_at RETURNING poll_id,participant_id,option_ids,updated_at",[poll.id,participant,JSON.stringify(selected),now()]);const r=rows[0];return {pollId:text(r,"poll_id"),participantId:text(r,"participant_id"),optionIds:(json(r,"option_ids") as string[])??[],updatedAt:text(r,"updated_at")}; },
  async getPollResults(id) { const pollRow=await getPollRow(id);if(!pollRow)return null;const poll=pollFromRow(pollRow);const responses=await query("SELECT option_ids FROM qwt_poll_responses WHERE poll_id=$1::uuid",[id]);return {poll,responseCount:responses.length,options:poll.options.map(option=>({...option,responseCount:responses.filter(r=>Array.isArray(json(r,"option_ids"))&&(json(r,"option_ids") as unknown[]).includes(option.id)).length}))}; },
  async archiveSessionActivity(code) { const row=await getSessionRow(code);if(!row)return null;const timestamp=now();const counts=await query(`WITH submissions AS (UPDATE qwt_submissions SET archived_at=$2,updated_at=$2 WHERE session_code=$1 AND archived_at IS NULL RETURNING 1), questions AS (UPDATE qwt_group_questions SET archived_at=$2,updated_at=$2 WHERE session_code=$1 AND archived_at IS NULL RETURNING 1) SELECT (SELECT count(*)::integer FROM submissions) AS submissions,(SELECT count(*)::integer FROM questions) AS questions`,[text(row,"id"),timestamp]);return {archivedAt:timestamp,submissions:number(counts[0],"submissions"),groupQuestions:number(counts[0],"questions")}; },
  async unarchiveSessionActivity(code,archivedAt) { const row=await getSessionRow(code);if(!row)return null;if(!Number.isFinite(Date.parse(archivedAt)))throw new Error("Archive timestamp could not be read.");const timestamp=now();const counts=await query(`WITH submissions AS (UPDATE qwt_submissions SET archived_at=NULL,updated_at=$3 WHERE session_code=$1 AND archived_at=$2::timestamptz RETURNING 1), questions AS (UPDATE qwt_group_questions SET archived_at=NULL,updated_at=$3 WHERE session_code=$1 AND archived_at=$2::timestamptz RETURNING 1) SELECT (SELECT count(*)::integer FROM submissions) AS submissions,(SELECT count(*)::integer FROM questions) AS questions`,[text(row,"id"),archivedAt,timestamp]);return {archivedAt,submissions:number(counts[0],"submissions"),groupQuestions:number(counts[0],"questions")}; },
};
