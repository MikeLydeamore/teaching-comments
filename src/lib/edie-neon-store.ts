...
function pollFromRow(row: Row): SessionPoll {
  ...
  return {
    ...
    startedAt: text(row, "started_at"), endsAt: text(row, "ends_at"), votingEndedAt: nullableText(row, "voting_ended_at"), endedAt: nullableText(row, "ended_at"), createdAt: text(row, "created_at"), updatedAt: text(row, "updated_at"),
  };
}
...
const POLL_COLUMNS = "id, session_code, question, selection_mode, options, correct_option_ids, solution_revealed, status, duration_seconds, started_at, ends_at, voting_ended_at, ended_at, created_at, updated_at";
...
export const neonStore: EdieStore = {
  ...
  async listSpaceMembers(spaceCode) {
    ...
    const rows = await query("SELECT space_code, user_id, role, created_at FROM edie_space_members WHERE space_code = $1 AND user_id IS NOT NULL ORDER BY user_id ASC", [normalized]);
      ...
      const rows = await query(
        `INSERT INTO edie_space_invitations (space_code, email, invitee_user_id, role)
         ...
         WHERE NOT EXISTS (
           ...
           WHERE space_code = $1 AND user_id = $3
         )
  ...
  async updateSpaceMemberRole(spaceCode, userId, role) {
    const normalized = normalizeSpaceCode(spaceCode); if (!normalized) return null;
    const rows = await query("UPDATE edie_space_members SET role = $3 WHERE space_code = $1 AND user_id = $2 RETURNING space_code, user_id, role, created_at", [normalized, userId, validateSpaceRole(role)]);
  ...
  async removeSpaceInvitation(spaceCode, email) {
    ...
    const rows = await query(
      `DELETE FROM edie_space_invitations
       WHERE space_code = $1 AND email = $2
       RETURNING space_code`,
  ...
  async startPoll(code,question,selectionMode,labels,correctOptionIndexes,durationSeconds) { const sessionRow=await getSessionRow(code);if(!sessionRow)return null;const session=sessionFromRow(sessionRow);if(!session.isOpen)throw new Error("This session is closed.");const d=validatePollDefinition(question,selectionMode,labels,durationSeconds);const timestamp=now();const options=d.optionLabels.map((label,position)=>({id:randomUUID(),label,position}));const correctOptionIds=validateCorrectOptionIndexes(d.selectionMode,d.optionLabels,correctOptionIndexes).map((index)=>options[index].id);const rows=await query(`WITH ended AS (UPDATE edie_polls SET status='ended',voting_ended_at=COALESCE(voting_ended_at,$2),ended_at=$2,updated_at=$2 WHERE session_code=$1 AND status='active') INSERT INTO edie_polls (session_code,question,selection_mode,options,correct_option_ids,solution_revealed,status,duration_seconds,started_at,ends_at,created_at,updated_at) VALUES ($1,$3,$4,$5::jsonb,$6::jsonb,false,'active',$7,$2,$8,$2,$2) RETURNING ${POLL_COLUMNS}`,[session.id,timestamp,d.question,d.selectionMode,JSON.stringify(options),JSON.stringify(correctOptionIds),d.durationSeconds,new Date(Date.parse(timestamp)+d.durationSeconds*1000).toISOString()]);return pollFromRow(rows[0]); },
  async extendPoll(id,seconds) { const extension=validatePollExtension(seconds);const row=await getPollRow(id);if(!row)return null;const poll=pollFromRow(row);if(poll.status!=="active")throw new Error("This poll has been ended.");if(poll.votingEndedAt||Date.parse(poll.endsAt)<=Date.now())throw new Error("This poll timer has ended.");const base=Math.max(Date.now(),Date.parse(poll.endsAt));const rows=await query(`UPDATE edie_polls SET duration_seconds=duration_seconds+$2,ends_at=$3,updated_at=$4 WHERE id=$1::uuid AND status='active' AND voting_ended_at IS NULL RETURNING ${POLL_COLUMNS}`,[id,extension,new Date(base+extension*1000).toISOString(),now()]);return rows[0]?pollFromRow(rows[0]):null; },
  async finishPoll(id) { const row=await getPollRow(id);if(!row)return null;const poll=pollFromRow(row);if(poll.status!=="active")throw new Error("This poll has been closed.");if(poll.votingEndedAt)return poll;const timestamp=now();const rows=await query(`UPDATE edie_polls SET voting_ended_at=$2,updated_at=$2 WHERE id=$1::uuid AND status='active' RETURNING ${POLL_COLUMNS}`,[id,timestamp]);return rows[0]?pollFromRow(rows[0]):null; },
  async endPoll(id) { const timestamp=now();const rows=await query(`UPDATE edie_polls SET status='ended',voting_ended_at=COALESCE(voting_ended_at,$2),ended_at=$2,updated_at=$2 WHERE id=$1::uuid RETURNING ${POLL_COLUMNS}`,[id,timestamp]);return rows[0]?pollFromRow(rows[0]):null; },
  async restartPoll(id) {
    const row = await getPollRow(id);
    if (!row) return null;
    const poll = pollFromRow(row);
    const isExpired = poll.status === "active" && (poll.votingEndedAt !== null || Date.parse(poll.endsAt) <= Date.now());
    const restartable = poll.status === "ended" || isExpired;
    if (!restartable) throw new Error("Only finished polls can be restarted.");
    const sessionRow = await getSessionRow(poll.sessionCode);
    if (!sessionFromRow((sessionRow ?? {}) as Row).isOpen) throw new Error("This session is closed.");
    const timestamp = now();
    const endsAt = new Date(Date.now() + poll.durationSeconds * 1000).toISOString();
    const rows = await query(
      `WITH ended AS (UPDATE edie_polls SET status='ended',voting_ended_at=COALESCE(voting_ended_at,$2),ended_at=$2,updated_at=$2 WHERE session_code=$3 AND status='active' AND id<>$1::uuid)
       UPDATE edie_polls SET status='active',ended_at=NULL,voting_ended_at=NULL,solution_revealed=false,ends_at=$4,updated_at=$2 WHERE id=$1::uuid AND (status='ended' OR voting_ended_at IS NOT NULL OR (status='active' AND ends_at <= $2::timestamptz)) RETURNING ${POLL_COLUMNS}`,
      [id, timestamp, poll.sessionCode, endsAt],
    );
