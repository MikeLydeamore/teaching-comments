...
export const neonStore: EdieStore = {
  ...
  async restartPoll(id) {
    ...
    const isExpired = poll.status === "active" && Date.parse(poll.endsAt) <= Date.now();
    ...
    const rows = await query(
      `WITH ended AS (UPDATE edie_polls SET status='ended',ended_at=$2,updated_at=$2 WHERE session_code=$3 AND status='active' AND id<>$1::uuid)
       UPDATE edie_polls SET status='active',ended_at=NULL,solution_revealed=false,ends_at=$4,updated_at=$2 WHERE id=$1::uuid AND (status='ended' OR (status='active' AND ends_at <= $2::timestamptz)) RETURNING ${POLL_COLUMNS}`,
